...
class Layers(object):
    ...
    def param_init_lstm_cond(self, options, params,
        ...
        alp_att = norm_weight(3,28)
        params[_p(prefix, 'alp_att')] = alp_att
        alpb_att = np.zeros((1,)).astype('float32')
        params[_p(prefix, 'alpb_att')] = alpb_att  # bo_lstm_c_att:(1,)
        if options['selector']:
            # attention: selector
    ...
    def lstm_cond_layer(self, tfparams, state_below, options, prefix='lstm',
                        mask=None, context=None, context_pca=None, one_step=False,
        ...
        with tf.name_scope("pctx_"):
            ...
            pctx_ = batch_matmul(context_pca, tfparams[_p(prefix, 'Wc_att')]) + tfparams[_p(prefix, 'b_att')]    # (64,3,2048)*(2048,2048)+(2048,) = (64,3,2048) or (1,512) in sampling
        # projected x
        ...
        alp_att = tfparams[_p(prefix, 'alp_att')]
        alpb_att = tfparams[_p(prefix, 'alpb_att')]
        ...
        def step(prev, elems):
            ...
            pctx_t = pctx_ + pstate_[:, None, :] # shape = (64,3,2048)+(64,?,2048) = (64,3,2048)  # DOUBT pctx_ += ?? VERIFIED
            # pctx_t = pctx_ + pstate_ # shape = (64,512)+(64,512) = (64,512)
                #   (1,28,2048) + (m,?,2048) = (m,28,2048)
            pctx_t = tanh(pctx_t)
            print "pctx_t shape: ", pctx_t.shape
            prev_alpha = batch_matmul(pctx_t, U_att) + alpb_att
            print "prev alpha shape: ", prev_alpha.shape
            prev_shape = tf.shape(prev_alpha)
            alpha = tf.reshape(prev_alpha,[prev_shape[0], prev_shape[1]])
            print "alpha shape: ", alpha.shape
            alpha = tf.expand_dims(tf.matmul(alpha,alp_att),-1) + c_att    # ((64,3,2048)*(2048,),1) + (1,) = (64,28,1) or (m,28,1) in sampling
            print "alpha shape: ", alpha.shape
            alpha_pre = alpha
