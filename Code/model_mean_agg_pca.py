...
from layers_mean_agg_pca import Layers
from collections import OrderedDict
import copy
import sys
from data_engine_mean_agg_pca import prepare_data
import utils
...
class Model(object):
    ...
    def build_model(self, tfparams, options, x, mask, ctx, ctx_mask, ctx_pca):
    ...
    def build_sampler(self, tfparams, options, use_noise, ctx0, ctx_mask, ctx_pca, x,
        ...
        bo_lstm = self.layers.get_layer('lstm_cond')[1](tfparams, emb, options,
                                                        ...
                                                        context_pca=tf.expand_dims(ctx_pca,0),
                                                        one_step=True,
    ...
    def gen_sample(self, sess, tfparams, f_init, f_next, ctx0, ctx_mask, ctx_pca, options,
    ...
    def pred_probs(self, sess, engine, whichset, f_log_probs, verbose=True):
        ...
        for index in iterator:
            ...
            x, mask, ctx, ctx_mask, ctx_pca = prepare_data(engine, tag, mode=whichset)
    ...
    def sample_execute(self, sess, engine, options, tfparams, f_init, f_next, x, ctx, ctx_mask, ctx_pca):
        ...
        for jj in xrange(np.minimum(10, x.shape[1])):
            sample, score, _, _ = self.gen_sample(sess, tfparams, f_init, f_next, ctx[jj], ctx_mask[jj], ctx_pca[jj],
