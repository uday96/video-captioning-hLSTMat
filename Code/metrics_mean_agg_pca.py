...
def generate_sample_gpu_single_process(sess,
    ...
    def sample(whichset):
        samples = []
        ctxs, ctx_masks, ctxs_pca = engine.prepare_data_for_blue(whichset)
        for i, ctx, ctx_mask, ctx_pca in zip(range(len(ctxs)), ctxs, ctx_masks, ctxs_pca):
            ...
            sample, score, _, _ = model.gen_sample(sess,
                None, f_init, f_next, ctx, ctx_mask, ctx_pca, options,
